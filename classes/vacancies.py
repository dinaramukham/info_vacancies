class Vacancy():
    ...
    def __init__(self, data, number_vacancy,
                 params):  # HeadHunter.change_params(text= 'аналитик') сначала вводим профессию
        ...
        try:  # если HeadHunter
            ...
            if Vacancy.get_vacancy(data, self.number_vacancy)['address'] is None:
                self.address = None
            else:
                self.address = Vacancy.get_vacancy(data, self.number_vacancy)['address']['raw']
            if Vacancy.get_vacancy(data, self.number_vacancy)['salary'] is None:
                self.salary = None
    ...
    def not_none(cls, one, two):
        ...
        if not isinstance(two, Vacancy):
            raise ValueError
        if one.salary is None:
            raise TypeError
        if two.salary is None:
            raise TypeError
    ...
    def __lt__(self, other):
        ...
        try:
            ...
            if self.salary['to'] is None and other.salary['from'] is None:
                return self.salary['from'] < other.salary['to']
            if self.salary['to'] is not None and other.salary['from'] is not None:
                return self.salary['to'] < other.salary['from']
            if self.salary['to'] is None:
                return self.salary['from'] < other.salary['from']
            if other.salary['from'] is None:
