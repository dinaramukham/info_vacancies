...
def get_course(currency='USD'):  # простой способ парсить курс
    """
    :param currency: 'USD' или 'RUB'
    :return: int курс
    """
    if currency == 'USD':  # сайт гугла доллар к рублю
    ...
    if currency == 'EUR':  # сайт гугла евро к рублю
    ...
    soup = BeautifulSoup(response.content, 'html.parser')  # создает объект BeautifulSoup
...
def get_translet(data, key):
    ...
    for el in range(len(copy_data)):
        if copy_data[el]['salary']['currency'] == 'EUR':  # 'EUR' USD
            ...
            copy_data[el]['salary'][key] = new_salary
        if copy_data[el]['salary']['currency'] == 'USD':
            new_salary = copy_data[el]['salary'][key] * get_course('USD')
            copy_data[el]['salary'][key] = new_salary
    return copy_data
